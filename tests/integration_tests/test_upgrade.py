...
from .network import Fury, setup_custom_fury
from .utils import (
    ADDRS,
    CONTRACTS,
    deploy_contract,
            ...
            ini[section].update(
                {
                    ...
                    "environment": (
                        f"DAEMON_NAME=fury,DAEMON_HOME=%(here)s/node{i}"
                    ),
                }
            )
...
def custom_fury(tmp_path_factory):
    ...
    yield from setup_custom_fury(
        ...
        post_init=post_init,
        chain_binary=str(path / "upgrades/genesis/bin/fury"),
    )


def test_cosmovisor_upgrade(custom_fury: Fury):
    """
    ...
    assert rsp["code"] == 0, rsp["raw_log"]
    # rsp = custom_fury.cosmos_cli(1).gov_vote("validator", proposal_id, "yes")
    # assert rsp["code"] == 0, rsp["raw_log"]
    ...
    custom_fury.chain_binary = (
        Path(custom_fury.chain_binary).parent.parent.parent
        / f"{plan_name}/bin/fury"
    )
    cli = custom_fury.cosmos_cli()

    # block should pass the target height
    wait_for_block(cli, target_height + 1, timeout=480)
    wait_for_port(ports.rpc_port(custom_fury.base_port(0)))

    # test migrate keystore
    cli.migrate_keystore()

    # check basic tx works after upgrade
    wait_for_port(ports.evmrpc_port(custom_fury.base_port(0)))
