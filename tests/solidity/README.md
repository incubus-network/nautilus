...
**Prerequisite**: in the repo's root, run `make install` to install the `fury` and `fury` binaries. When done, come back to this directory.
...
To run the tests, you can use the `test-helper.js` utility to test all suites under `ganache` or `fury` network. The `test-helper.js` will help you spawn an `fury` process before running the tests.

You can simply run `yarn test --network fury` to run all tests with fury network, or you can run `yarn test --network ganache` to use ganache shipped with truffle. In most cases, there two networks should produce identical test results.

If you only want to run a few test cases, append the name of tests following by the command line. For example, use `yarn test --network fury basic` to run the `basic` test under `fury` network.

If you need to take more control, you can also run `fury` using:
...
Keep the terminal window open, go into any of the tests and run `yarn test-fury`. You should see `fury` accepting transactions and producing blocks. You should be able to query for any transaction via:

- `fury query tx <cosmos-sdk tx>`
...
The [`init-test-node.sh`](./init-test-node.sh) script sets up fury with the following accounts:
