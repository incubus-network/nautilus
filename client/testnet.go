...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(ethermint.AttovBlack, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(ethermint.AttovBlack, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(ethermint.AttovBlack)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, ethermint.AttovBlack, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
