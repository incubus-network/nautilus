...
"$PWD"/build/nautid keys add $KEY --keyring-backend test --algo "eth_secp256k1"

# Set moniker and chain-id for Ethermint (Moniker can be anything, chain-id must be an integer)
"$PWD"/build/nautid init $MONIKER --chain-id $CHAINID
...
"$PWD"/build/nautid add-genesis-account "$("$PWD"/build/nautid keys show "$KEY" -a --keyring-backend test)" 100000000000000000000avblack,10000000000000000000stake --keyring-backend test

# Sign genesis transaction
"$PWD"/build/nautid gentx $KEY 10000000000000000000stake --amount=100000000000000000000avblack --keyring-backend test --chain-id $CHAINID

# Collect genesis tx
"$PWD"/build/nautid collect-gentxs

# Run this to ensure everything worked and that the genesis file is setup correctly
"$PWD"/build/nautid validate-genesis

# Start the node (remove the --pruning=nothing flag if historical queries are not needed) in background and log to file
"$PWD"/build/nautid start --pruning=nothing --rpc.unsafe --json-rpc.address="0.0.0.0:8545" --keyring-backend test > nautid.log 2>&1 &

# Give nautid node enough time to launch
sleep 5
...
#PRIVKEY="$("$PWD"/build/nautid keys export $KEY)"
...
# After tests
# kill test nautid
echo "going to shutdown nautid in 3 seconds..."
sleep 3
pkill -f "ethermint*"