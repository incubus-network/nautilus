...
## default port prefixes for nautid
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    ...
    "$PWD"/build/nautid keys add $KEY"$i" --home "$DATA_DIR$i" --no-backup --chain-id $CHAINID --algo "eth_secp256k1" --keyring-backend test
    ...
    "$PWD"/build/nautid validate-genesis --home "$DATA_DIR$i"
}

start_func() {
    echo "starting ethermint node $i in background ..."
    "$PWD"/build/nautid start --pruning=nothing --rpc.unsafe \
