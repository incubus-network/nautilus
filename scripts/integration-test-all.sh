...
init_func() {
    ...
    "$("$PWD"/build/fury keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000axfury,1000000000000000000stake \
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test fury node rpc=$HOST_RPC ws=$HOST_WS ..."
