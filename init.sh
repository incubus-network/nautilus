...
if [[ ! -f "$GENESIS_FILE" ]]; then
  ...
  cat $HOME/.fury/config/genesis.json | jq '.app_state["staking"]["params"]["bond_denom"]="axfury"' > $HOME/.fury/config/tmp_genesis.json && mv $HOME/.fury/config/tmp_genesis.json $HOME/.fury/config/genesis.json
  ...
  # increase block time (?)
  cat $HOME/.fury/config/genesis.json | jq '.consensus_params["block"]["time_iota_ms"]="1000"' > $HOME/.fury/config/tmp_genesis.json && mv $HOME/.fury/config/tmp_genesis.json $HOME/.fury/config/genesis.json
...
fury validate-genesis $GENESIS_FILE
