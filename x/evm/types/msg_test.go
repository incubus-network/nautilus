...
func (suite *MsgsTestSuite) TestMsgEthereumTx_BuildTx() {
	...
	for _, tc := range testCases {
		...
		tx, err := tc.msg.BuildTx(suite.clientCtx.TxConfig.NewTxBuilder(), "avblack")
		if tc.expError {
			...
			suite.Require().Equal(sdk.NewCoins(sdk.NewCoin("avblack", sdkmath.NewInt(100000))), tx.GetFee())
		}
	}
}
