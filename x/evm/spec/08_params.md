...
For example, on Ethereum, the `evm_denom` would be `ETH`. In the case of Fury, the default denomination is the **[atto xfury](notion://www.notion.so/docs/basics/xfury.md)** (used on the Evmos testnets). In terms of precision, the `XFURY` and `ETH` share the same value, *i.e* `1 XFURY = 10^18 atto xfury` and `1 ETH = 10^18 wei`.
